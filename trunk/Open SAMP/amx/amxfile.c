 ...
 *
 *  Version: $Id: amxfile.c,v 1.1 2008-09-17 07:33:22 kyecvs Exp $
 */
...
static char *completename(wchar_t *dest, TCHAR *src, size_t size)
{
  #if defined AMXFILE_VAR
    TCHAR *prefix,*ptr;
    size_t len;
...
{
	TCHAR *name, *newname,fullname[_MAX_PATH], newfullname[_MAX_PATH];

	amx_StrParam(amx,params[1],name);
	amx_StrParam(amx,params[2],newname);
	if (name!=NULL && completename(fullname,name,sizeof fullname)!=NULL && newname!=NULL && completename(newfullname,newname,sizeof newfullname)!=NULL)
	{
