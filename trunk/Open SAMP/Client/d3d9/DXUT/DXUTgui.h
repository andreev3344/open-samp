...
// Added by Kye of the SA-MP Team 2009. Define a few colours that allow us
// to easily change the texture fills for most controls
//#define DXUT_CONTROL_COLOUR_SELECTION		D3DCOLOR_ARGB(240, 160, 60, 70)
#define DXUT_CONTROL_COLOUR_SELECTION		D3DCOLOR_XRGB(185, 34, 40)
#define DXUT_CONTROL_COLOUR_PRESSED			D3DCOLOR_XRGB(149, 176, 208)
#define EVENT_LISTBOX_TAB		            0x0710
...
#define DXUT_LISTBOX_MAX_COLUMNS		3

struct DXUTListBoxItem
{
	#pragma pack( 1 )
    TCHAR strText[256];
	TCHAR strColumnText[DXUT_LISTBOX_MAX_COLUMNS][64]; // added by Kye of the SA-MP team.
    void  *pData;
    RECT  rcActive;
    bool  bSelected;
	DWORD dwItemColor;
...
private:
	...
	int		m_nColumns;		// Number of additional data columns in the list (0=none)
	LONG	m_lColumnOffsets[DXUT_LISTBOX_MAX_COLUMNS]; // Offset locations for additional columns
...
public:
    ...
    virtual HRESULT OnInit() { 
		//OutputDebugString("CDXUTListBox::OnInit() Initing Scrollbar");
    ...
    HRESULT AddItem( const TCHAR *wszText, void *pData, DWORD dwColorItemColor = 0 );
    HRESULT InsertItem( int nIndex, const TCHAR *wszText, void *pData, DWORD dwColorItemColor = 0 );
	...
	void SetColumnCount( int nColumnCount ) {
		m_nColumns = nColumnCount; 
	};

	void SetColumnOffset( int nColumnId, LONG lOffsetX ) {
		m_lColumnOffsets[nColumnId] = lOffsetX;
	};
