...
void CDXUTDialog::InitDefaultElements()
{
    ...
    SetFont( 0, "Arial", 20, FW_BOLD );
	SetFont( 1, "Arial", 18, FW_BOLD );
    ...
    Element.TextureColor.States[ DXUT_STATE_PRESSED ] = DXUT_CONTROL_COLOUR_PRESSED;
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = DXUT_CONTROL_COLOUR_SELECTION;
	...
	// Addition by Kye of the SA-MP Team:
	// use same texture for button fill
	SetRect( &rcTexture, 0, 0, 136, 54 );
    ...
    Element.TextureColor.States[ DXUT_STATE_NORMAL ] = D3DCOLOR_ARGB(200, 255, 255, 255);
	Element.TextureColor.States[ DXUT_STATE_MOUSEOVER ] = DXUT_CONTROL_COLOUR_SELECTION;
    Element.TextureColor.States[ DXUT_STATE_FOCUS ] = DXUT_CONTROL_COLOUR_SELECTION;
	...
	Element.TextureColor.Init(DXUT_CONTROL_COLOUR_SELECTION);
    ...
    Element.SetTexture( 0, &rcTexture );
    Element.TextureColor.Init(DXUT_CONTROL_COLOUR_SELECTION, DXUT_CONTROL_COLOUR_PRESSED);
    ...
    m_ScrollBar( pDialog )
{
	...
	// Added by Kye of the SA-MP team.
	m_nColumns = 0;

	for( int i=0; i < DXUT_LISTBOX_MAX_COLUMNS; ++i )
	{
		m_lColumnOffsets[i] = 0L;
	}
}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTListBox::AddItem( const TCHAR *wszText, void *pData, DWORD dwColor )
{
	...
	pNewItem->dwItemColor = dwColor;

	for( int i=0; i < DXUT_LISTBOX_MAX_COLUMNS; ++i )
	{
		memset((PVOID)&pNewItem->strColumnText[i][0],0,64);
	}
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTListBox::InsertItem( int nIndex, const TCHAR *wszText, void *pData, DWORD dwColor )
{
    DXUTListBoxItem *pNewItem = new DXUTListBoxItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    switch( uMsg )
    {
        ...
        case WM_LBUTTONDBLCLK:
            ...
            if( m_Items.GetSize() > 0 && PtInRect( &m_rcSelection, pt ) )
            {
                ...
                {
                    SetCapture( DXUTGetHWND() );
                    //m_bDrag = true; // Kye : SA-MP. Don't want this behaviour.
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < (int)m_Items.GetSize(); ++i )
        {
            ...
            if( bSelectedStyle )
            {
				...
				if( m_nColumns > 0 )
				{
					...
					for( cols=0; cols < m_nColumns; ++cols )
					{
						rc.left += m_lColumnOffsets[cols];
						m_pDialog->DrawText( pItem->strColumnText[cols], pSelElement, &rc );
					}
			...
			{
                if(pItem->dwItemColor) 
				{
					pElement->FontColor.Current = pItem->dwItemColor;
				}
				...
				if( m_nColumns > 0 )
				{
					...
					{
						rc.left += m_lColumnOffsets[cols];
						m_pDialog->DrawText( pItem->strColumnText[cols], pElement, &rc );
					}
