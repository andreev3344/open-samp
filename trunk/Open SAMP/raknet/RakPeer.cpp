...
#ifndef _CLIENT_MOD
extern unsigned long serverTicket;
#endif
...
void RakPeer::OnConnectionRequest( RakPeer::RemoteSystemStruct *remoteSystem, unsigned char *AESKey, bool setAESKey )
{
	...
	//if ( AllowIncomingConnections() )
	{
		...
		RakNet::BitStream bitStream(sizeof(unsigned char)+sizeof(unsigned short)+sizeof(unsigned int)+sizeof(unsigned short)+sizeof(PlayerIndex)+sizeof(unsigned int)); // Open SA:MP
...
//#else
//		bitStream.Write((unsigned short)myPlayerId.port);  // Open SA:MP
//#endif
...
#ifndef _CLIENT_MOD
		bitStream.Write(serverTicket); // Open SA:MP
#endif
...
RakPeer::RemoteSystemStruct * RakPeer::AssignPlayerIDToRemoteSystemList( const PlayerID playerId, RemoteSystemStruct::ConnectMode connectionMode )
{
	...
	for ( i = 0; i < maximumNumberOfPeers; i++ ) // Open SA:MP
	{
...
#ifndef _CLIENT_MOD
unsigned int lastBinaryAddress; // Open SA:MP
RakNetTime lastConnectionTick; // Open SA:MP
void logprintf(char* str, ...);
#endif

/*	Open SA:MP	*/
unsigned short RakPeer::GetNumberOfActiveConnections( void ) const
{
...
unsigned short RakPeer::GetNumberOfUnverifiedSystems( unsigned int binAddr )
{
	unsigned short i = 0, count = 0;

	if ( binAddr == 0 || remoteSystemList == 0 || endThreads == true )
		return 0;

	for ( i = 0; i < maximumNumberOfPeers; i++ )
	{
		if (remoteSystemList[ i ].isActive && remoteSystemList[ i ].playerId.binaryAddress == binAddr )
			if(remoteSystemList[ i ].connectMode == RemoteSystemStruct::UNVERIFIED_SENDER)
				count++;
	}
...
//		char data[MAX_OFFLINE_DATA_LENGTH];
	...
	{
		if(*((WORD*)&data[1]) != (WORD)((SocketLayer::Instance()->GetLocalPort(rakPeer->connectionSocket)) ^ 5))
			return;

		for (i=0; i < rakPeer->messageHandlerList.Size(); i++)
			rakPeer->messageHandlerList[i]->OnDirectSocketReceive(data, length*8, playerId);
		
		const char* remoteIP = rakPeer->PlayerIDToDottedIP(playerId);
		bool isFlood = false;
#ifndef _CLIENT_MOD
		if(binaryAddress == lastBinaryAddress)
		{
			if(rakPeer->GetNumberOfUnverifiedSystems(binaryAddress) >= 30)
			{
				logprintf("Blocking %s due to a 'server full' attack (1)", remoteIP);
				rakPeer->AddToBanList(remoteIP, 120000);
				isFlood = true;
			}
		}
		else if((RakNet::GetTime() - lastConnectionTick) < 10000)
		{
			unsigned short maxPeers = rakPeer->GetMaximumNumberOfPeers() - 1;
			if(rakPeer->GetNumberOfActiveConnections() == maxPeers)
			{
				logprintf("Blocking %s due to a 'server full' attack (2)", remoteIP);
				rakPeer->AddToBanList(remoteIP, 120000);
				isFlood = true;
			}
		}
		...
		if (rss==0 || rss->weInitiatedTheConnection==true)
		{
			...
			SocketLayer::Instance()->SendTo( rakPeer->connectionSocket, (char*)&c, 2, playerId.binaryAddress, playerId.port );
#ifndef _CLIENT_MOD
			lastConnectionTick = RakNet::GetTime();
			lastBinaryAddress = binaryAddress;

			logprintf("Incoming connection: %s:%u", remoteIP, port);
#endif
			return;
		}
		else if (rss!=0)
