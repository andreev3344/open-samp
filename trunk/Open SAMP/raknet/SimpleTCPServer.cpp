...
#ifdef _WIN32
unsigned __stdcall UpdateTCPServerLoop( LPVOID arguments );
#else
#define closesocket close
void* UpdateTCPServerLoop( void* arguments );
#endif
...
bool SimpleTCPServer::Start(unsigned short port)
{
	if (isStarted)
		return false;
...
#ifdef _WIN32
	unsigned threadId = 0;
	threadHandle = ( HANDLE ) _beginthreadex( NULL, 0, UpdateTCPServerLoop, this, 0, &threadId );
...
//	pthread_attr_setschedparam(&attr, &sp);
	...
	int error;
	error = pthread_create( &threadHandle, &attr, &UpdateTCPServerLoop, this );
	if ( error )
		return false;
#endif
...
void SimpleTCPServer::Stop(void)
{
	if (isStarted==false)
		return;
...
void SimpleTCPServer::Send( const char *data, unsigned length, PlayerID playerId )
{
	if (isStarted==false)
		return;
...
Packet* SimpleTCPServer::Receive( void )
{
	if (isStarted==false)
		return 0;
	return incomingMessages.ReadLock();
}
void SimpleTCPServer::CloseConnection( PlayerID playerId )
{
	if (isStarted==false)
		return;
...
void SimpleTCPServer::DeallocatePacket( Packet *packet )
{
	assert(incomingMessages.CheckReadUnlockOrder(packet));
	delete [] packet->data;
	incomingMessages.ReadUnlock();
}
PlayerID SimpleTCPServer::HasNewConnection(void)
{
	PlayerID *out;
...
PlayerID SimpleTCPServer::HasLostConnection(void)
{
	PlayerID *out;
...
#endif
void SimpleTCPServer::DeleteRemoteClient(RemoteClient *remoteClient, fd_set *exceptionFD)
{
...
#endif
{
	SimpleTCPServer * sts = ( SimpleTCPServer * ) arguments;
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
		...
		if (selectResult > 0)
		{
			if (FD_ISSET(sts->listenSocket, &readFD))
			{
