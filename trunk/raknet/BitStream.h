...
/// \note  __BITSTREAM_NATIVE_END should be defined by default for speed.  Turn it off if you actually need endian swapping
...
#define BITS_TO_uint8_tS(x) (((x)+7)>>3)
	...
	// For "error: explicit specialization in non-namespace scope" in gcc
	#ifdef _MSC_VER
	#if _MSC_VER < 1299 // VC6
		...
		/// Write an objectID to a bitstream
		/// \param[in] var The value to write
		template <>
			void Write(ObjectID var);

		/// Write an objectID.  If the current value is different from the last value
		/// the current value will be written.  Otherwise, a single bit will be written
		/// \param[in] currentValue The current value to write
		/// \param[in] lastValue The last value to compare against
		template <>
			void WriteDelta(ObjectID currentValue, ObjectID lastValue);
		...
		template <>
			void WriteCompressed(ObjectID var);
		...
		/// Read an ObjectID from a bitstream
		/// \param[in] var The value to read
		template <>
			bool Read(ObjectID &var);
		...
		template <>
			bool ReadCompressed(ObjectID &var);
...
#pragma warning(disable:4127)   // conditional expression is constant
	...
	/// \param[in] var The value to write
	template <>
		inline void BitStream::Write(ObjectID var)
	{
		if (ObjectID::peerToPeerMode)
			Write(var.playerId);
	...
	/// \param[in] lastValue The last value to compare against
	template <>
		inline void BitStream::WriteDelta(ObjectID currentValue, ObjectID lastValue)
	{
		if (currentValue==lastValue)
		{
			Write(false);
...
#pragma warning(disable:4244)   // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
	...
	template <>
		inline void BitStream::WriteCompressed(ObjectID var)
	{
		Write(var);
	}
...
#pragma warning(disable:4244)   // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
	...
	/// \param[in] var The value to read
	template <>
		inline bool BitStream::Read(ObjectID &var)
	{
		if (ObjectID::peerToPeerMode)
			Read(var.playerId);
...
#pragma warning(disable:4127)   // conditional expression is constant
	...
	template <>
		inline bool BitStream::ReadCompressed(ObjectID &var)
	{
		return Read(var);
	}
